...
impl LexicalEnvironment {
    pub fn new(global: JsObject) -> Self {
        let _timer = Profiler::global().start_event("LexicalEnvironment::new", "env");
...
impl Context {
    ...
    pub(crate) fn initialize_binding(&mut self, name: Sym, value: JsValue) -> JsResult<()> {
        let _timer =
            Profiler::global().start_event("LexicalEnvironment::initialize_binding", "env");
        self.get_current_environment()
    ...
    pub(crate) fn get_current_environment(&mut self) -> Environment {
        let _timer =
            Profiler::global().start_event("LexicalEnvironment::get_current_environment", "env");
        self.realm
            .environment
            .environment_stack
            .back_mut()
    ...
    pub(crate) fn has_binding(&mut self, name: Sym) -> JsResult<bool> {
        let _timer = Profiler::global().start_event("LexicalEnvironment::has_binding", "env");
        self.get_current_environment()
